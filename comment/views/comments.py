...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        #send email for article's author & user & user's parent comment
        ...
        if self.comment.parent:
            ...
            if parent_email in [author_email, user_email]:
                parent_email = False
        if author_email:
            email = EmailMessage('ديدگاه جديد', 'ديدگاه جديد براي مقاله {} كه شما نويسنده آن هستيد، ارسال شده\n {}{}'.format(article, current_site, reverse('article:article-detail', kwargs={'slug': article.slug}))
            , to= [author_email])
            email.send()

        if user_email:
            email = EmailMessage('ديدگاه شما دريافت شد', 'ديدگاه شما دريافت شد و به زودي بررسي خواهد شد.'
            , to= [user_email])
            email.send()

        if parent_email:
            email = EmailMessage('پاسخي به ديدگاه شما', 'پاسخي به ديدگاه شما براي مقاله {} ارسال شده، از لينك زير مي توانيد مشاهده كنيد:\n {}{}'.format(article, current_site, reverse('article:article-detail', kwargs={'slug': article.slug}))
            , to= [parent_email])
            email.send()
